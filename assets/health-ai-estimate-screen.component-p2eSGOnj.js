...
`;var h=class extends HTMLElement{_imageFile=null;_previewUrl=null;_imagePath=null;_model=``;_foodItemId=null;_defaultDate=null;_defaultMealType=null;constructor(){super();let e=this.attachShadow({mode:`open`});e.adoptedStyleSheets=[p],e.appendChild(m.content.cloneNode(!0))}connectedCallback(){let e=this.shadowRoot;e.getElementById(`btn-photo`).addEventListener(`click`,()=>{e.getElementById(`photo-input`).click()}),e.getElementById(`photo-input`).addEventListener(`change`,e=>{let t=e.target;t.files?.[0]&&this.loadImage(t.files[0])}),e.getElementById(`btn-remove-photo`).addEventListener(`click`,()=>{this.clearImage()}),e.getElementById(`btn-estimate`).addEventListener(`click`,()=>{this.doEstimate()}),e.getElementById(`btn-retry`).addEventListener(`click`,()=>{this.showInputView()}),e.getElementById(`btn-save`).addEventListener(`click`,()=>{this.doSave()}),e.getElementById(`btn-error-back`).addEventListener(`click`,()=>{this.showInputView()}),this.applyDefaults()}set defaultDate(e){this._defaultDate=e&&e.trim()?e:null,this.applyDefaults()}set defaultMealType(t){if(!t||!e.includes(t)){this._defaultMealType=null,this.applyDefaults();return}this._defaultMealType=t,this.applyDefaults()}loadImage(e){let t=this.shadowRoot;this._imageFile=e,this._imagePath=null,this._previewUrl&&URL.revokeObjectURL(this._previewUrl),this._previewUrl=URL.createObjectURL(e);let n=t.getElementById(`photo-preview`);n.src=this._previewUrl,n.alt=`Selected meal photo`,n.hidden=!1,t.getElementById(`btn-remove-photo`).hidden=!1}clearImage(){let e=this.shadowRoot;this._imageFile=null,this._imagePath=null,this._previewUrl&&=(URL.revokeObjectURL(this._previewUrl),null);let t=e.getElementById(`photo-preview`);t.hidden=!0,t.src=``,e.getElementById(`btn-remove-photo`).hidden=!0,e.getElementById(`photo-input`).value=``}showInputView(){let e=this.shadowRoot;e.getElementById(`input-view`).hidden=!1,e.getElementById(`loading-view`).hidden=!0,e.getElementById(`result-view`).hidden=!0,e.getElementById(`error-view`).hidden=!0}async doEstimate(){let e=this.shadowRoot,t=e.getElementById(`description`).value.trim();if(!t&&!this._imageFile){e.getElementById(`description`).focus();return}e.getElementById(`input-view`).hidden=!0,e.getElementById(`loading-view`).hidden=!1;let n=e.getElementById(`loading-text`);n.textContent=`Analyzing your meal…`;let r=setTimeout(()=>{n.textContent=`Starting AI engine… this may take a moment`},5e3);try{if(this._imageFile&&!this._imagePath){let{image_path:e}=await s.uploadImage(`ai`,this._imageFile,this._imageFile.name||`meal.jpg`);this._imagePath=e}let e=await this.requestEstimate(t||`meal in photo`,this._imagePath??void 0);clearTimeout(r),this._model=e.ai_estimation.model_name,this._foodItemId=e.food_item.id,this._imagePath=e.food_item.image_path??this._imagePath,this.showResult(e.estimate)}catch(e){clearTimeout(r);let t=e instanceof Error?e.message:`Unknown error`;this.showError(t)}}showResult(e){let t=this.shadowRoot;t.getElementById(`loading-view`).hidden=!0,t.getElementById(`result-view`).hidden=!1;let n=t.getElementById(`result-image`);this._imagePath?(n.src=s.variantUrl(this._imagePath,`display.jpg`),n.hidden=!1):(n.hidden=!0,n.removeAttribute(`src`)),t.getElementById(`food-name`).value=e.display_name,t.getElementById(`serving-form`).data=l(e);let r=t.getElementById(`confidence-badge`),i=e.confidence_score>=.75?`high`:e.confidence_score>=.5?`medium`:`low`;r.textContent=i,r.className=`confidence-badge confidence-${i}`;let a=t.getElementById(`notes-text`);a.textContent=e.notes||``,a.hidden=!e.notes}showError(e){let t=this.shadowRoot;t.getElementById(`loading-view`).hidden=!0,t.getElementById(`error-view`).hidden=!1,t.getElementById(`error-text`).textContent=e}doSave(){let e=this.shadowRoot,t=e.getElementById(`food-name`).value.trim(),n=e.getElementById(`meal-type`).value,r=e.getElementById(`log-date`).value;if(!t||!this._foodItemId)return;let{servingAmount:i,serving:a}=e.getElementById(`serving-form`).getCurrentValues();this.dispatchEvent(d({food_item_id:this._foodItemId,amount:i,meal_type:n,date:r,calories:a.calories,protein_g:a.protein_g,carbs_g:a.carbs_g,fat_g:a.fat_g,fiber_g:a.fiber_g,sugar_g:a.sugar_g}))}async requestEstimate(e,t){let n=u({description:e,image_path:t,respondWith:null});if(this.dispatchEvent(n),!n.detail.respondWith)throw Error(`Estimate handler not set`);return await n.detail.respondWith}reset(){this.showInputView();let e=this.shadowRoot;e.getElementById(`description`).value=``,this.clearImage();let t=e.getElementById(`photo-preview`);t.hidden=!0,t.src=``,t.alt=`Preview`,this.applyDefaults()}applyDefaults(){let e=this.shadowRoot;if(!e)return;let t=e.getElementById(`log-date`);t&&(t.value=this._defaultDate??Temporal.Now.plainDateISO().toString());let n=e.getElementById(`meal-type`);n&&(n.value=this._defaultMealType??c())}};customElements.define(`ai-estimate`,h);var g=class extends HTMLElement{initialized=!1;dateValue=null;mealValue=null;log=t(`health-ai-estimate-screen`);setLoading(){}setError(e){}clearStatus(){}async returnToOrigin(){await n([`/health/food-entry`,`/health/scanner`,`/health/ai-estimate`],this.dateValue?`/health/day/${encodeURIComponent(this.dateValue)}`:`/health`)}set date(e){this.dateValue=e}set meal(e){this.mealValue=e}connectedCallback(){if(this.initialized)return;this.initialized=!0;let e=r(this,`Health`,`health`,`/health`),t=document.createElement(`ai-estimate`);t.defaultDate=this.dateValue,t.defaultMealType=this.mealValue,t.addEventListener(f.ESTIMATE_NUTRITION,e=>{e.detail.respondWith=o.estimateNutrition(e.detail.description,e.detail.image_path)}),t.addEventListener(f.LOG_AI_ESTIMATE,async e=>{let t=e.detail;return a(this,this.log,async()=>{await o.createConsumptionEntry(t),await this.returnToOrigin()})}),e.content.appendChild(t)}};customElements.define(`health-ai-estimate-screen`,g);export{g as HealthAiEstimateScreen};
//# sourceMappingURL=health-ai-estimate-screen.component-p2eSGOnj.js.map