...
`;var v={manual:`Manual`,openfoodfacts:`Open Food Facts`,ai_estimate:`AI Estimate`},y=class extends HTMLElement{_entry=null;_onSave=null;_onDelete=null;constructor(){super();let e=this.attachShadow({mode:`open`});e.adoptedStyleSheets=[g],e.appendChild(_.content.cloneNode(!0))}connectedCallback(){let e=this.shadowRoot;e.getElementById(`btn-save`).addEventListener(`click`,()=>this.handleSave()),e.getElementById(`btn-delete`).addEventListener(`click`,()=>this.handleDelete())}set onSave(e){this._onSave=e}set onDelete(e){this._onDelete=e}set entry(e){this._entry=e,this.populateForm()}get entry(){return this._entry}populateForm(){let e=this._entry;if(!e)return;let t=this.shadowRoot;t.getElementById(`source-badge`).textContent=v[e.source]??e.source;let n=t.getElementById(`food-name`);n.value=e.food_name??``,n.readOnly=e.source===`openfoodfacts`,t.getElementById(`meal-type`).value=e.meal_type;let r=t.getElementById(`serving-form`);r.data=s(e)}handleSave(){if(!this._entry||!this._onSave)return;let e=this.shadowRoot,t=e.getElementById(`serving-form`),{servingAmount:n,serving:r}=t.getCurrentValues(),i=t._data?.servingByWeight??!1,a={food_name:e.getElementById(`food-name`).value.trim(),meal_type:e.getElementById(`meal-type`).value,...r};i&&(a.serving_size=`${Math.round(n)}g`),this._onSave(this._entry.id,a)}handleDelete(){!this._entry||!this._onDelete||confirm(`Delete "${this._entry.food_name}"?`)&&this._onDelete(this._entry.id)}};customElements.define(`nutrition-edit`,y);var b=t(`:host{display:block}.edit-container{flex-direction:column;gap:16px;max-width:600px;margin:0 auto;padding:16px;display:flex}.edit-header h3{color:var(--text-primary,#fff);margin:0;font-size:1.1rem}.form-group{flex-direction:column;gap:4px;display:flex}.form-group label{color:var(--text-secondary,#aaa);font-size:.85rem}.form-input,.form-select{border:1px solid var(--border-color,#444);background:var(--surface-color,#1e1e1e);color:var(--text-primary,#fff);box-sizing:border-box;border-radius:8px;padding:10px 12px;font-size:.95rem}.form-row{gap:12px;display:flex}.form-row .form-group{flex:1}.action-buttons{gap:8px;display:flex}.btn-delete{color:#f44336;cursor:pointer;background:0 0;border:1px solid #f44336;border-radius:8px;flex:1;padding:12px;font-size:.95rem}.btn-delete:hover{background:#f443361a}.btn-save{background:var(--accent-color,#d4a017);color:#000;cursor:pointer;border:none;border-radius:8px;flex:2;padding:12px;font-size:.95rem;font-weight:600}.btn-save:hover{opacity:.9}`),x=document.createElement(`template`);x.innerHTML=`
    <div class="edit-container">
        <div class="edit-header">
            <h3>Edit Workout</h3>
        </div>
...
`;var S=class extends HTMLElement{_activity=null;_onSave=null;_onDelete=null;constructor(){super();let e=this.attachShadow({mode:`open`});e.adoptedStyleSheets=[b],e.appendChild(x.content.cloneNode(!0))}connectedCallback(){let e=this.shadowRoot;e.getElementById(`btn-save`).addEventListener(`click`,()=>this.handleSave()),e.getElementById(`btn-delete`).addEventListener(`click`,()=>this.handleDelete())}set onSave(e){this._onSave=e}set onDelete(e){this._onDelete=e}set activity(e){this._activity=e,this.populateForm()}get activity(){return this._activity}populateForm(){let e=this._activity;if(!e)return;let t=this.shadowRoot;t.getElementById(`title`).value=e.title,t.getElementById(`type`).value=e.type,t.getElementById(`date`).value=e.date,t.getElementById(`duration`).value=e.duration_minutes?String(e.duration_minutes):``,t.getElementById(`calories`).value=e.calories_burned?String(e.calories_burned):``,t.getElementById(`description`).value=e.description??``}handleSave(){if(!this._activity||!this._onSave)return;let e=this.shadowRoot,t=e.getElementById(`title`).value.trim();if(!t)return;let n=parseInt(e.getElementById(`duration`).value,10),r=parseInt(e.getElementById(`calories`).value,10),i={title:t,type:e.getElementById(`type`).value,date:e.getElementById(`date`).value,description:e.getElementById(`description`).value.trim()||void 0,duration_minutes:isNaN(n)?void 0:n,calories_burned:isNaN(r)?void 0:r};this._onSave(this._activity.id,i)}handleDelete(){!this._activity||!this._onDelete||confirm(`Delete "${this._activity.title}"?`)&&this._onDelete(this._activity.id)}};customElements.define(`activity-edit`,S);var C=document.createElement(`template`);C.innerHTML=`
