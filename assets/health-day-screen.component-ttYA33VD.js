...
`;var p=class extends HTMLElement{_activity=null;constructor(){super();let e=this.attachShadow({mode:`open`});e.adoptedStyleSheets=[u],e.appendChild(f.content.cloneNode(!0))}set activity(e){this._activity=e,this.render()}formatDuration(e){if(!e)return``;if(e>=60){let t=Math.floor(e/60),n=e%60;return n>0?`${t}h ${n}min`:`${t}h`}return`${e} min`}render(){let e=this._activity;if(!e)return;let t=this.shadowRoot;t.getElementById(`type-icon`).textContent=s(e.type),t.getElementById(`title`).textContent=e.title;let n=[],r=this.formatDuration(e.duration_minutes);r&&n.push(r),e.calories_burned&&n.push(`${Math.round(e.calories_burned)} kcal`),e.description&&n.push(e.description),t.getElementById(`meta`).textContent=n.join(` · `);let i=t.getElementById(`source-badge`);i.textContent=d[e.source]??`❓`,i.title=e.source.charAt(0).toUpperCase()+e.source.slice(1)}};customElements.define(`activity-detail-card`,p);var m=t(l),h=document.createElement(`template`);h.innerHTML=`
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <select id="type" class="form-select">
                    ${i.map(e=>`<option value="${e}">${e.charAt(0).toUpperCase()+e.slice(1)}</option>`).join(``)}
                </select>
            </div>
            <div class="form-group">
    ...
    <dialog class="activity-dialog" id="activity-dialog">
        <form method="dialog" id="activity-form">
            ...
            <div class="form-group">
                ...
                <select id="act-type">
                    ${i.map(e=>`<option value="${e}">${e.charAt(0).toUpperCase()+e.slice(1).replace(`-`,` `)}</option>`).join(``)}
                </select>
            </div>
            <div class="form-group">
...
`;var T=class extends HTMLElement{initialized=!1;detail=null;loadSequence=0;dateValue=``;dialogRoot=null;set date(e){this.dateValue=e}connectedCallback(){if(!this.initialized){this.initialized=!0;let e=r(this,`Health`,`health`,`/health`);this.detail=document.createElement(`health-day-detail`),this.detail.dateContext=this.dateValue,this.detail.showLoading(),e.content.appendChild(this.detail),this.dialogRoot=document.createElement(`div`),e.content.appendChild(this.dialogRoot),this.dialogRoot.appendChild(w.content.cloneNode(!0)),this.wireDetail(),this.wireAddDialog()}this.detail&&(this.detail.dateContext=this.dateValue),this.loadData()}wireDetail(){this.detail&&(this.detail.addEventListener(n.ADD_WORKOUT,e=>{let{detail:t}=e;this.openManualAddDialog(t.date)}),this.detail.addEventListener(n.ADD_FOOD,t=>{let{detail:n}=t;e(`/health/food-entry/${encodeURIComponent(n.date)}/${encodeURIComponent(n.meal)}`)}),this.detail.addEventListener(n.EDIT_FOOD,e=>{let{detail:t}=e;this.showNutritionEditor(t.entry)}),this.detail.addEventListener(n.EDIT_WORKOUT,e=>{let{detail:t}=e;this.showActivityEditor(t.activity)}))}wireAddDialog(){let e=this.querySelector(`#activity-dialog`),t=this.querySelector(`#activity-form`);!e||!t||e.addEventListener(`close`,()=>{if(e.returnValue!==`save`){t.reset();return}let n=this.querySelector(`#act-title`)?.value??``,r=this.querySelector(`#act-type`)?.value??`other`,i=this.querySelector(`#act-date`)?.value??``,a=this.querySelector(`#act-duration`)?.value??``,o=a?parseInt(a,10):void 0;if(!n||!i){t.reset();return}this.saveManualActivity({title:n,type:r,date:i,duration_minutes:o}),t.reset()})}async saveManualActivity(e){try{await o.createActivity(e),await this.loadData()}catch(e){console.error(`Failed to create activity`,e),this.detail?.showError(`Failed to save activity.`)}}openManualAddDialog(e){let t=this.querySelector(`#activity-dialog`),n=this.querySelector(`#act-date`);!t||!n||(n.value=e,t.showModal())}async loadData(){if(!this.detail)return;let e=++this.loadSequence;this.detail.dateContext=this.dateValue,this.detail.showLoading();try{let[t,n]=await Promise.all([o.fetchActivitiesByDate(this.dateValue),o.fetchNutritionByDate(this.dateValue)]);if(e!==this.loadSequence||!this.detail)return;this.detail.activities=t,this.detail.nutrition=n,this.detail.hideLoading(),this.detail.showContent()}catch(t){console.error(`Failed to load day detail data`,t),e===this.loadSequence&&this.detail.showError(`Failed to load detail for ${this.dateValue}`)}}showNutritionEditor(e){if(!this.detail||!this.dialogRoot)return;this.detail.hidden=!0;let t=document.createElement(`nutrition-edit`);t.entry=e,t.onSave=async(e,n)=>{try{await o.updateNutritionEntry(e,n),t.remove(),this.detail.hidden=!1,await this.loadData()}catch(e){console.error(`Failed to update nutrition entry`,e)}},t.onDelete=async e=>{try{await o.deleteNutritionEntry(e),t.remove(),this.detail.hidden=!1,await this.loadData()}catch(e){console.error(`Failed to delete nutrition entry`,e)}},this.dialogRoot.appendChild(t)}showActivityEditor(e){if(!this.detail||!this.dialogRoot)return;this.detail.hidden=!0;let t=document.createElement(`activity-edit`);t.activity=e,t.onSave=async(e,n)=>{try{await o.updateActivity(e,n),t.remove(),this.detail.hidden=!1,await this.loadData()}catch(e){console.error(`Failed to update activity`,e)}},t.onDelete=async e=>{try{await o.deleteActivity(e),t.remove(),this.detail.hidden=!1,await this.loadData()}catch(e){console.error(`Failed to delete activity`,e)}},this.dialogRoot.appendChild(t)}};customElements.define(`health-day-screen`,T);export{T as HealthDayScreen};
//# sourceMappingURL=health-day-screen.component-ttYA33VD.js.map